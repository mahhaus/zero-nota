...
@SpringBootTest(classes = ZeroNotaApplication.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class ZeroNotaAPITests {
	@Autowired
	...
	private ResponseEntity<CarroDTO> getCarro(String url) {
		return
				rest.withBasicAuth("josias","luiza.2014").getForEntity(url, CarroDTO.class);
	}

	private ResponseEntity<List<CarroDTO>> getCarros(String url) {
		return rest.withBasicAuth("josias","luiza.2014").exchange(
				url,
				HttpMethod.GET,
				null,
	...
	public void testSave() {
		...
		// Insert
		ResponseEntity response = rest.withBasicAuth("admin","luiza.2014").postForEntity("/api/v1/carros", carro, null);
		...
		// Deletar o objeto
		rest.withBasicAuth("admin","luiza.2014").delete(location);
